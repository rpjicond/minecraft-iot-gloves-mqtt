...
const char *mqtt_username = "jouer1"; 
...
#define TOPIC_INDEX "gant1/index"
#define TOPIC_MAJEUR  "gant1/majeur"
#define TOPIC_ANNUAIRE "gant1/annulaire"
#define TOPIC_AURICULAIRE "gant1/auriculaire"
 ...
 while (!client.connected()) {
    ...
    if (client.connect(mqtt_client_id,mqtt_username,mqtt_password)){ 
      Serial.print("Connecte au MQTT broker \n");
        ...
        Serial.print("La connexion au MQTT broker a échoué");
         //client.state()=4 : MQTT_CONNECT_BAD_CREDENTIALS
...
void loop() {
  ...
  if (digitalRead(BP_INDEX) == LOW) {
    if (currentTime - lastPressTimeIndex > debounceDelay) {
      Serial.println("Gant1 BP_Index pressé");
  ...
  if (digitalRead(BP_MAJEUR) == LOW) {
    if (currentTime - lastPressTimeMajeur > debounceDelay) {
      Serial.println("Gant1 BP_majeur pressé");
  ...
  if (digitalRead(BP_ANNUAIRE) == LOW) {
    if (currentTime - lastPressTimeAnnuaire > debounceDelay) {
      Serial.println("Gant1 BP_annuaire pressé");
  ...
  if (digitalRead(BP_AURICULAIRE) == LOW) {
    if (currentTime - lastPressTimeAuriculaire > debounceDelay) {
      Serial.println("Gant1 BP_auriculaire pressé");
